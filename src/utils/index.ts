...
export function getEtherscanLink(
  ...
  let prefix = '';
  if(chainId === 513100){
    prefix= `https://explorer.etherfair.org`
  }
  else if (chainId !== 256 && chainId !== 128) {
    prefix = `https://${ETHERSCAN_PREFIXES[chainId] || ETHERSCAN_PREFIXES[1]}etherscan.io`
  } else if(chainId===256 || chainId===128){
    prefix = `https://${ETHERSCAN_PREFIXES[chainId]}hecoinfo.com`
  } else{
    prefix= `https://explorer.etherfair.org`
  }
...
export function isETFChain(chainId: ChainId | undefined): boolean {
  return chainId === 513100
}
