...
const CurrencySelect = styled.button<{ selected: boolean }>`
  ...
  font-size: 20px;
  font-weight: 500;
  background-color: ${({ selected, theme }) => (selected ? theme.bg1 : theme.primary7)};
  ...
  :focus,
  :hover {
    background-color: ${({ selected, theme }) => (selected ? theme.bg2 : theme.primary7)};
  }
`
              ...
              {pair ? (
                ...
                <StyledTokenName className="token-symbol-container" active={Boolean(currency && currency.symbol)}>
                  {(symbol && symbol.length > 20
                    ? symbol.slice(0, 4) +
                      '...' +
                      symbol.slice(symbol.length - 5, symbol.length)
                    : symbol) || t('selectToken')}
                </StyledTokenName>
              )}
